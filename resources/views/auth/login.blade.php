...
<body style="margin: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #000000; height: 100vh; display: flex; align-items: center; justify-content: center; position: relative; overflow: hidden;">
    ...
    <div style="background-color: #000000; padding: 40px; border-radius: 4px; box-shadow: 0 0 30px rgba(0, 0, 0, 0.5); width: 100%; max-width: 360px; transform: translateY(0); transition: all 0.3s ease; animation: fadeIn 0.6s ease-out; border: 1px solid rgba(0, 255, 85, 0.2); position: relative; z-index: 10;">
        ...
        <div style="margin-bottom: 30px;">
            <h2 style="margin: 0; color: #ffffff; font-weight: 500; font-size: 24px; letter-spacing: 1px;">WELCOME BACK</h2>
            <p style="margin-top: 8px; color: #999999; font-size: 14px;">Please enter your credentials to continue</p>
        </div>
        ...
        <form method="POST" action="{{ route('login') }}" style="display: flex; flex-direction: column;">
            ...
            <button 
                type="submit" 
                style="background-color: #00ff55; color: #000000; border: none; padding: 14px; font-size: 14px; font-weight: 600; cursor: pointer; transition: all 0.3s; text-transform: uppercase; letter-spacing: 1px;">
                Sign In
            </button>
        </form>

        <div style="text-align: center; margin-top: 25px; padding-top: 20px; border-top: 1px solid #222222;">
            <a href="{{ route('register') }}" style="color: #00ff55; text-decoration: none; font-size: 14px; transition: color 0.3s;">Don't have an account? <span style="text-decoration: underline;">Sign up</span></a>
        </div>
    </div>
